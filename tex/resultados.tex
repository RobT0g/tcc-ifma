...
O processo iterativo demonstrou-se extremamente eficiente para a modelagem do sistema, especialmente quando comparado a metodologias alternativas de desenvolvimento e 
testes de software embarcado. Isso é demonstrado em diferentes perspectivas, dentre as quais é importamnte destacar a possibilidade da validação do sistema antes 
mesmo da escrita ou geração do código.

A validação aplicada apenas no sistema físico traz uma complicação que não só atrasa a entrega do produto físico, mas também torna 
o processo mais caro. Isso deve-se ao fato de que atingir o nível de maturidade do sistema em que é possível executar testes no hardware normalmente só é alcançado 
meses após a conclusão da etapa de análise, em que o design do sistema é definido.

Até que a maturidade seja alcançada e o design seja colocado a prova, o trabalho de implementação já foi executado em grande parte, custando várias horas de trabalho 
de engenheiros de software. Qualquer problema encontrado nos testes desta fase vão precisar passar por extensos processos de identificação e precisarão ser comunicados 
ao engenheiro de sistema responsável pela análise. 

Diante deste cenário, um possível erro no design que passou despercebido durante a definição do sistema se torna extremamente caro, pois a sua identificação só foi 
possível após meses de trabalho de vários engenheiros de diferentes departamentos. Além do mais, para conseguir definir uma solução, é necessário que os engenheiros 
revisem o design, proponham uma mudança que deve ser implementada e testada novamente, o que gera um grande custo de horas de retrabalho.  

No caso da metodologia proposta por este trabalho, a execução dos testes pode ser feita antes mesmo do desenvolvimento do código, podendo até estar sob a 
responsabilidade do mesmo engenheiro que realiza a análise do sistema. Isso faz com que ele tenha uma maior confiabilidade sobre o seu design e tenha o 
controle sobre quaisquer mudanças que venham a ser necessárias. Quanto mais cedo uma falha for descoberta no desenvolvimento de um sistema, mais barato é 
a sua solução.

Um exemplo que ilustra claramente essa eficiência ocorreu durante a execução dos testes da história de usuário 3, quando foi constatado a necessidade do uso de uma 
condição de tempo para validar a condição em que o feedback deveria ser realizado. A princípio, sem visualizar a execução do sistema, o tempo não foi considerado 
como uma condição necessária, pois a necessidade de validar a saída do sistema, não apenas o pressionar do botão, não foi levantada.

Outro benefício desta metodologia é a facilidade na identificação da causa dos erros dos testes, como no caso das três primeiras falhas da história de usuário 3.
...
dificultaria a detecção das falhas observadas.
