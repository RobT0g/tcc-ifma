...
O trabalho também será ampliado com a investigação de novas metodologias de desenvolvimento, de modo a possibilitar comparações mais precisas e fundamentadas. Além 
disso, determinadas técnicas poderão ser integradas ao processo, permitindo que a adaptação final apresente maior qualidade ao incorporar elementos provenientes de 
diferentes abordagens metodológicas.

Outro aspecto de incremento refere-se à integração com técnicas de Inteligência Artificial, visando identificar potenciais melhorias de eficiência na execução do 
processo. Entre as possibilidades, destaca-se o uso de IA Generativa para ampliar funcionalidades e cobertura de testes, por meio da análise e sugestão de alterações 
em cenários descritos em \textit{Gherkin}, bem como a utilização de Agentes baseados em \textit{Large Language Models} (LLM) capazes de acelerar etapas de implementação.
